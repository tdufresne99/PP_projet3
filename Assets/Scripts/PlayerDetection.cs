...
public class PlayerDetection : MonoBehaviour
{
    ...
    [SerializeField] private float _radius; // Define the radius of the check
    [SerializeField] private float _distanceThreshold;
    [SerializeField] private Transform _playerTransform;
    [SerializeField] private Transform _motherTransform;
    [SerializeField] private LayerMask _wallsLayerMask;

    void Update()
    {
        DetectPlayer(transform, _playerTransform);
    }
    ...
    private void DetectPlayer(Transform objectTransform, Transform otherObjectTransform)
    {
        ...
        RaycastHit hit;
        bool isHit = Physics.Raycast(object1Pos, direction, out hit, _distanceThreshold, _wallsLayerMask);

        // Check if the raycast hit anything
        if (!isHit || hit.collider.gameObject == _playerTransform.gameObject)
        {
